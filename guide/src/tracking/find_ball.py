...
def detectBall(frame):
    ...
    for (i,c) in enumerate(cnts):
        ...
        if area > 1000 and perimeter > 100:
            ...
            X.append(cX)
            Y.append(cY)
            netPerim.append(perimeter)

    # Average out each x & y location determined
    if X and Y:
        cX = int(sum(X)/len(X))
        cY = int(sum(Y)/len(Y))
        cNetPerim = int(sum(netPerim)/len(netPerim))
        return cX, cNetPerim, mask
    else:
        return -100, 1234, mask

# Callback called whenever image received
def image_callback(data):
	global cX, cY, pub, cPm
	...
	cX, cPm, mask = detectBall(image)

	# Create Point instance and set x, y methods
	point = Point()
	point.x = cX
	point.y = cPm # distance metric
...
if __name__ == '__main__':
	global counter, X, Y, netPerim, cX, cY, pub
	counter = 0
	X, Y, netPerim = [], [], []
	...
	img_sub = rospy.Subscriber("tb3_2/camera/rgb/image_raw/compressed",\
